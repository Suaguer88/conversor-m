//Declarar variables para capturar la información

let rate1 = document.querySelector('.rate1');
let rate2 = document.querySelector('.rate2');
let reslutBtn = document.querySelector('.result');

let selects = document.querySelectorAll('.options select')
let sel1 = selects[0];
let sel2 = selects[1];

let inputs = document.querySelectorAll('.input input');
let inpt1 = inputs[0];
let inpt2 = inputs[1];

let rates = {};
let requestUrl = "https://api.exchangerate.host/latest?base=USD";

fetchRates();

async function fetchRates(){
    let res = await fetch(requestUrl);
    res = await res.json();
    rates = res.rates;

    populateOptions();
}

function populateOptions(){
    let val = '';
    Object.keys(rates).forEach(code => {
        let str = `<option value="${code}">${code}</option>`;
        //incrementar el option
        val += str;
    })
    //mostrar las opciones

    selects.forEach((s)=> (s.innerHTML = val));
}

//Operación matematica para obtener la conversión

/**
 * 
 * @param {Number} val valor de conversión
 * @param {Number} fromCurr  moneda inicial
 * @param {Number} toCurr  moneda convertir
 */

function convert(val, fromCurr, toCurr){
    //Variables de ambito local
    let v = (val/rates[fromCurr]) * rates[toCurr];
    let v1 = v.toFixed(3);

    return v1 == 0.0 ? v1.toFixed(5) : v1;
}

// Crear función para los select

function displayRate(){
    let v1 = sel1.value;
    let v2 = sel2.value;

    let val = convert(1,v1, v2);
    // imprimir el elemento html
    rate1.innerHTML = `1 ${v1} equals `;
    rate2.innerHTML = `${val} ${v2}`;
}

//listener y evento click

reslutBtn.addEventListener("click", ()=>{
    let fromCurr = sel1.value;
    let fromVal = parseFloat(inpt1.value);
    let toCurr = sel2.value;

    if(isNaN(fromVal)){
        alert("Por favor Ingresa un número");
    }else{
        let cVal = convert(fromVal,fromCurr,toCurr);
        inpt2.value = cVal;
    }
});

selects.forEach(s => s.addEventListener("change", displayRate));

document.querySelector('.swap').addEventListener("click",()=>{
    let in1 = inpt1.value;
    let in2 = inpt2.value;
    let op1 = sel1.value;
    let op2 = sel2.value;

    inpt2.value = in1;
    inpt1.value = in2;

    sel2.value = op1;
    sel1.value = op2;

    displayRate();
});